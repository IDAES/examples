...
class _GasPhaseStateBlock(StateBlock):
    ...
    def initialize(
        ...
        for k in blk.keys():
            if hasattr(blk[k], "sum_component_eqn"):
...
class GasPhaseStateBlockData(StateBlockData):
    ...
    def _mole_frac_comp_max(self):
        eps = 2e-6

        def mole_frac_comp_max_expr(b, i):
            return smooth_max(b.mole_frac_comp[i], 0, eps)

        try:
            self.mole_frac_comp_max = Expression(
                self._params.component_list, rule=mole_frac_comp_max_expr
            )
        except AttributeError:
            self.del_component(self.mole_frac_comp_max)
            raise
    ...
    def _mw(self):
        ...
        def mw_eqn(b):
            return b.mw == sum(
                b.mole_frac_comp_max[j] * b._params.mw_comp[j]
    ...
    def _dens_mol(self):
        ...
        def ideal_gas(b):
            ...
            return dens_mol * temperature_smooth * gas_constant == pressure
    ...
    def _dens_mol_comp(self):
        ...
        def comp_conc_eqn(b, j):
            return b.dens_mol_comp[j] == b.dens_mol * b.mole_frac_comp_max[j]
    ...
    def _visc_d(self):
        ...
        def visc_d_comp(i):
            ...
            eps = 1e-8 * pyunits.K  # epsilon parameter for smoothing
            temperature_smooth = (self.temperature**2 + eps**2) ** 0.5
            return (
                visc_d_param_1
                * (temperature_smooth ** self._params.visc_d_param_2[i])
                / (
                    (1 + (self._params.visc_d_param_3[i] / temperature_smooth))
                    + (self._params.visc_d_param_4[i] / (temperature_smooth**2))
                )
            )
    ...
    def _therm_cond(self):
        ...
        def therm_cond_comp(i):
            ...
            return (
                therm_cond_param_1
                * (temperature_smooth ** self._params.therm_cond_param_2[i])
                / (
                    (1 + (self._params.therm_cond_param_3[i] / temperature_smooth))
                    + (self._params.therm_cond_param_4[i] / (temperature_smooth**2))
                )
            )
    ...
    def _cp_mol_comp(self):
        ...
        def pure_component_cp_mol(b, j):
            ...
            eps = 1e-8 * pyunits.kK  # epsilon parameter for smoothing
            t_smooth = (t**2 + eps**2) ** 0.5
            return b.cp_mol_comp[j] == pyunits.convert(
                (
                    b._params.cp_param_1[j]
                    + b._params.cp_param_2[j] * t_smooth
                    + b._params.cp_param_3[j] * t_smooth**2
                    + b._params.cp_param_4[j] * t_smooth**3
                    + b._params.cp_param_5[j] / (t_smooth**2)
                ),
    ...
    def _cp_mol(self):
        ...
        def cp_mol(b):
            return b.cp_mol == sum(
                b.cp_mol_comp[j] * b.mole_frac_comp_max[j]
                for j in b._params.component_list
            )
    ...
    def _enth_mol_comp(self):
        ...
        def pure_comp_enthalpy(b, j):
            ...
            return b.enth_mol_comp[j] == pyunits.convert(
                ...
                b._params.cp_param_1[j] * t_smooth
                + b._params.cp_param_2[j] * (t_smooth**2) / 2
                + b._params.cp_param_3[j] * (t_smooth**3) / 3
                + b._params.cp_param_4[j] * (t_smooth**4) / 4
                - b._params.cp_param_5[j] / (t_smooth),
    ...
    def _entr_mol(self):
        ...
        def entropy_correlation(b):
            ...
            return (
                ...
                * b.mole_frac_comp[j]
                * (
                    self.params.cp_param_1[j] * log(t_smooth / pyunits.kK)
                    + self.params.cp_param_2[j] * t_smooth
                    + self.params.cp_param_3[j] * t_smooth**2 / 2
                    + self.params.cp_param_4[j] * t_smooth**3 / 3
                    - self.params.cp_param_5[j] / t_smooth**2 / 2
    ...
    def calculate_scaling_factors(self):
        ...
        if self.is_property_constructed("material_density_terms"):
            for i, c in self.material_density_terms.items():
                ...
                iscale.set_scaling_factor(c, 1e-2 * sf1 * sf2)
        ...
        if self.is_property_constructed("dens_mass_basis"):
            sf = iscale.get_scaling_factor(self.dens_mol)
            iscale.constraint_scaling_transform(
                self.dens_mass_basis, sf, overwrite=False
            )
